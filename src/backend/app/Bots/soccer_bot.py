...
llm_smart = ChatGoogleGenerativeAI(model="gemini-2.0-flash-lite", temperature=0, max_retries=2,request_timeout=30)
...
try:
    ...
    # Crear Motor SQL en Memoria (SQLite)
    engine = create_engine("sqlite:///:memory:")
    
    # Volcar el DataFrame a la tabla 'players'
    df.to_sql(name="players", con=engine, index=False)
